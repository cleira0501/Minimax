...
class TicTacToeBoard:
    ...
    def _check_row_win(self, player, board):
        """
        Check whether the given player has won along a row of the board.

        Args:
            player: A string representing the player's symbol (X or O).
            board: A list of lists representing the board to check.

        Returns:
            True if player has won along any row of the board and False otherwise.
        """
        for row in range(3):
    ...
    def _check_col_win(self, player, board):
        """
        ...
        Returns:
            True if player has won along any column of the board and False otherwise.
        """
        for col in range(3):
    ...
    def _check_diag_win(self, player, board):
        """
        ...
        Returns:
            True if player has won along any diagonal of the board and False otherwise.
        """
        trace_win = True
        for i in range(3):
            if board[i][i] != player:
    ...
    def check_win(self, player, board_state=None):
        ...
        """
        board = board_state if board_state else self._board
        return (
            self._check_row_win(player, board)
            or self._check_col_win(player, board)
            or self._check_diag_win(player, board)
        )
    def is_full(self, board_state=None):
        """
        Check if the board is completely filled (no blank squares left).

        Args:
            board_state: An optional list of lists representing a board to check instead
                of the internal board.

        Returns:
            True if the board has no blank spaces and False otherwise.
        """
        board = board_state if board_state else self._board
        for i in range(3):
            for j in range(3):
                if board[i][j] == self.blank_mark:
                    return False
        return True
